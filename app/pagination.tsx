  ...
  return (
    <>
      <div className="flex flex-row justify-center mt-5">
        ...
        <input name='page_index' type='number' max={max_page} value={target_page} className="text-black mx-2 w-24" onChange={(event) => setTargetPage(Math.min(Number(event.target.value), max_page))} />
      </div>
    </>
  );
}
